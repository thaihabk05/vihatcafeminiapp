...
import { getServerData } from "utils/config";
...
export const categoriesState = selector<Category[]>({
  key: "categories",
  get: () => getServerData().categories as Category[],
});
...
export const productsState = selector<Product[]>({
  ...
  get: async () => {
    const variants = (await import("../mock/variants.json"))
      .default as Variant[];
    const products = getServerData().products as any[];
    return products.map(
      (product) =>
        ({
          ...product,
          variants: variants.filter(
            (variant) =>
              Array.isArray(product.variantId) &&
